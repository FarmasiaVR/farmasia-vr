    ...
    public void OnTrashEnter(Collider other) {
        ...
        if (item != null) {
            if (!normalTrash.Contains(item.ObjectType) && !sharpTrash.Contains(item.ObjectType)) {
                ...
                if (trashType == TrashType.Sharp && normalTrash.Contains(item.ObjectType)) Task.CreateGeneralMistake("Normaali esine laitettiin ter�vien roskikseen", 1, true);
                if (trashType == TrashType.Normal && sharpTrash.Contains(item.ObjectType)) Task.CreateGeneralMistake("Ter�v� esine laitettiin normaaliin roskikseen", 1, true);
            }
